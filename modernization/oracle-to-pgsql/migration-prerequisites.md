...
CREATE USER MIGRATIONUSER IDENTIFIED BY Microsoft123456789
DEFAULT TABLESPACE users
TEMPORARY TABLESPACE temp
QUOTA UNLIMITED ON users;
/
-- Core privileges for migration
GRANT CREATE SESSION TO MIGRATIONUSER;
GRANT SELECT ANY TABLE TO MIGRATIONUSER;
GRANT SELECT ANY SEQUENCE TO MIGRATIONUSER;
GRANT SELECT_CATALOG_ROLE TO MIGRATIONUSER;
GRANT SELECT ANY DICTIONARY TO MIGRATIONUSER;
...
END;
/
-- Note the slash (/) above — it executes the PL/SQL block

--CLEANUP (DROP EXISTING MIGRATION ROLE IF EXISTS)
BEGIN
   EXECUTE IMMEDIATE 'DROP ROLE MIGRATION_ROLE';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE = -01919 THEN 
         DBMS_OUTPUT.PUT_LINE('MIGRATION_ROLE does not exist — skipping drop.');
      ELSE 
         RAISE;
      END IF;
END;
